...
def find_projective_correspondence(source_points,
    ...
    print("System finding projective correspondences")
    ...
    mask = (target_us >=0) & (target_us < w) & (target_vs >= 0) & (target_vs < h) & (T_source_points[:, 2] > 0)
    # End of TODO
    ...
    mask = np.zeros_like(target_us).astype(bool)
    mask = np.abs(target_ds - T_source_points[:, 2]) < dist_diff
    # End of TODO
...
def build_linear_system(source_points, target_points, target_normals, T):
    print("System building linear system")
    M = len(source_points)
...
def pose2transformation(delta):
    ...
    print("System doing pose2transform")
    w = delta[:3]
...
def solve(A, b):
    ...
    # LU = splu(ATA.tocsc(), permc_spec='NATURAL')
...
def icp(source_points,
    ...
    '''
    print("System finding ICP")
    T = T_init

    for i in range(10):
        # TODO: fill in find_projective_correspondences
        print("System doing iteration", i)
